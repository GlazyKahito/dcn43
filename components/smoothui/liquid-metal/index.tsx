...
export type LiquidMetalVariant = "chrome" | "gold" | "mercury" | "oil" | "emerald";
...
export interface LiquidMetalProps {
  ...
  /** Track window pointer movements for global background mode */
  globalTracking?: boolean;
}

const DEFAULT_SPEED = 0.8;
const DEFAULT_DISTORTION = 1.2;
const MAX_DPR = 1.75;
...
const FALLBACK_SURFACE: Record<LiquidMetalVariant, string> = {
  chrome:
    "conic-gradient(from 210deg at 50% 50%, #080a0d, #64748b, #cbd5e1, #475569, #94a3b8, #0f172a, #64748b, #080a0d)",
  ...
  emerald:
    "conic-gradient(from 210deg at 50% 50%, #030705, #064e3b, #34d399, #047857, #6ee7b7, #022c22, #10b981, #030705)",
};
...
const TEXT_SURFACE: Record<LiquidMetalVariant, string> = {
  ...
  emerald:
    "linear-gradient(100deg, #064e3b 0%, #34d399 16%, #10b981 32%, #ecfdf5 46%, #059669 62%, #6ee7b7 78%, #022c22 100%)",
};
...
void main(){
  ...
  // Chrome base palette: dark obsidian to gleaming silver-white
  vec3 lo = vec3(0.02, 0.03, 0.04);
  vec3 hi = vec3(0.85, 0.90, 0.96);
  vec3 tint = vec3(0.40, 0.50, 0.62);
  ...
  if (uVariant > 0.5 && uVariant < 1.5) {
    ...
    // Mercury
    lo = vec3(0.03, 0.04, 0.06);
    hi = vec3(0.96, 0.98, 1.00);
    tint = vec3(0.45, 0.52, 0.65);
    ...
    // Emerald Chrome (Cybersecurity Lab signature)
    lo = vec3(0.015, 0.035, 0.025);
    hi = vec3(0.70, 0.95, 0.85);
    tint = vec3(0.18, 0.80, 0.55);
  }

  vec3 col = mix(lo, hi, smoothstep(0.04, 0.96, f));
  col = mix(col, tint, pow(sideways, 2.0) * 0.45);

  float spec = pow(max(dot(n, normalize(vec3(0.35, 0.75, 0.55))), 0.0), 38.0);
  float fresnel = pow(1.0 - clamp(n.z, 0.0, 1.0), 3.0);
  col += spec * 0.75 + fresnel * 0.18;

  if (uVariant > 2.5 && uVariant < 3.5) {
...
export const LiquidMetal = ({
  ...
  // Pointer tracking (local or global)
  useEffect(() => {
    if (!pointerInfluence || shouldReduceMotion) {
      return;
    }
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      if (globalTracking) {
        // Global tracking across the whole window
        const width = window.innerWidth || 1;
        const height = window.innerHeight || 1;
        x = (event.clientX / width - POINTER_CENTER) * POINTER_RANGE;
        y = (POINTER_CENTER - event.clientY / height) * POINTER_RANGE;
      } else {
        const wrapper = wrapperRef.current;
        if (!wrapper) return;
        const rect = wrapper.getBoundingClientRect();
        x = ((event.clientX - rect.left) / rect.width - POINTER_CENTER) * POINTER_RANGE;
        y = (POINTER_CENTER - (event.clientY - rect.top) / rect.height) * POINTER_RANGE;
      }
    ...
    if (globalTracking) {
      ...
      wrapper.addEventListener("pointerleave", handleLeave, { passive: true });
      return () => {
        wrapper.removeEventListener("pointermove", handlePointerMove);
        wrapper.removeEventListener("pointerleave", handleLeave);
      };
    }
  }, [globalTracking, pointerInfluence, shouldReduceMotion]);
  ...
  if (maskText) {
    ...
    return (
      <span
        ...
        ref={(node) => {
          if (node) wrapperRef.current = node as unknown as HTMLDivElement;
        }}
      >
    ...
    >
      {usesShader ? (
        <div aria-hidden="true" className="absolute inset-0 z-0 pointer-events-none">
          <canvas className="h-full w-full block" ref={canvasRef} />
        </div>
      ) : (
        <div
          aria-hidden="true"
